...
# ==============================================================================
# 1. PAGE CONFIGURATION & ENTERPRISE REACT-STYLE CSS
# ==============================================================================
st.set_page_config(
...
<style>
    ...
    .stApp { background: #F4F7F9 !important; } /* Soft grey background like the image */
    ...
    /* Wondermove-Style Dashboard CSS Injection */
    .wm-card {
        background: #FFFFFF;
    ...
    .stTextInput>div>div>input, .stNumberInput>div>div>input, .stSelectbox>div>div>div {
        height: 42px !important; font-size: 0.9rem !important; font-weight: 500 !important;
    ...
    .stButton>button { height: 42px !important; font-weight: 600 !important; border-radius: 6px !important; font-size: 0.9rem !important;}
    ...
    .section-header {
        ...
        border-bottom: 1px solid #E5E7EB !important; padding-bottom: 6px !important;
        margin: 20px 0 16px 0 !important; text-transform: uppercase !important; letter-spacing: 0.5px !important;
    }

    div[data-testid="stDataFrame"] > div { border-radius: 8px !important; border: 1px solid #E5E7EB !important; }
</style>
""", unsafe_allow_html=True)

# ==============================================================================
# 2. CORE SYSTEM FUNCTIONS & DATABASE
# ==============================================================================
...
def generate_settlement_pdf(driver_name, start_d, end_d, trips_df, adv_df, totals):
    ...
    pdf.add_page()
    pdf.set_font("Arial", 'B', 16); pdf.cell(0, 8, "KSS Roadways - Settlement Statement", ln=True, align='C')
    pdf.set_font("Arial", '', 11); pdf.cell(0, 6, f"Driver: {driver_name} | Period: {start_d} to {end_d}", ln=True, align='C'); pdf.ln(5)
    
    pdf.set_font("Arial", 'B', 11); pdf.set_fill_color(240, 245, 255); pdf.cell(0, 8, " Overall Position", ln=True, fill=True)
    pdf.set_font("Arial", '', 10)
    pdf.cell(60, 8, f" Diesel: {totals['diesel']:.1f} L", border=1); pdf.cell(65, 8, f" Bata Earned: Rs {totals['bata']:.2f}", border=1)
    pdf.cell(65, 8, f" Advance Ded: Rs {totals['adv']:.2f}", border=1, ln=True)
    pdf.set_font("Arial", 'B', 10); pdf.cell(190, 8, f" Final Balance Payable: Rs {totals['bal']:.2f}", border=1, align='C', ln=True); pdf.ln(5)
    
    pdf.set_font("Arial", 'B', 11); pdf.cell(0, 8, " Trip Details", ln=True)
    if not trips_df.empty:
        pdf.set_font("Arial", 'B', 8)
        cols = ["Date", "LR No", "Truck", "Route", "Diesel", "Bata", "Adv", "Bal"]
        widths = [20, 15, 22, 58, 15, 18, 18, 24]
        for i in range(len(cols)): pdf.cell(widths[i], 7, cols[i], border=1, align='C')
        pdf.ln(); pdf.set_font("Arial", '', 8)
        for _, row in trips_df.iterrows():
            pdf.cell(widths[0], 7, str(row.get('trip_start_date', ''))[:10], border=1)
            pdf.cell(widths[1], 7, str(row.get('lr_no', ''))[:8], border=1)
            pdf.cell(widths[2], 7, str(row.get('vehicle_number', ''))[:10], border=1)
            pdf.cell(widths[3], 7, f"{str(row.get('source',''))[:10]} to {str(row.get('destination',''))[:10]}", border=1)
            pdf.cell(widths[4], 7, f"{row.get('diesel_litres', 0):.1f}", border=1, align='R')
            pdf.cell(widths[5], 7, f"{row.get('total_bata', 0):.0f}", border=1, align='R')
            pdf.cell(widths[6], 7, f"{row.get('advance_issued', 0):.0f}", border=1, align='R')
            pdf.cell(widths[7], 7, f"{row.get('balance_amount', 0):.0f}", border=1, align='R'); pdf.ln()
    else: pdf.set_font("Arial", '', 9); pdf.cell(0, 8, "No trips logged.", ln=True)
    
    pdf.ln(5); pdf.set_font("Arial", 'B', 11); pdf.cell(0, 8, " Direct Advances", ln=True)
    if not adv_df.empty:
        pdf.set_font("Arial", 'B', 8)
        pdf.cell(30, 7, "Date", border=1, align='C'); pdf.cell(30, 7, "Amount (Rs)", border=1, align='C')
        pdf.cell(40, 7, "Category", border=1, align='C'); pdf.cell(90, 7, "Remarks", border=1, align='C'); pdf.ln()
        pdf.set_font("Arial", '', 8)
        for _, row in adv_df.iterrows():
            pdf.cell(30, 7, str(row.get('advance_date', ''))[:10], border=1); pdf.cell(30, 7, f"{row.get('amount_inr', 0):.2f}", border=1, align='R')
            pdf.cell(40, 7, str(row.get('advance_type', ''))[:20], border=1); pdf.cell(90, 7, str(row.get('reference_remarks', ''))[:45], border=1); pdf.ln()
    else: pdf.set_font("Arial", '', 9); pdf.cell(0, 8, "No direct advances.", ln=True)
...
if not st.session_state.authenticated:
    ...
    with col:
        st.markdown("<h2 style='text-align: center; color: #111827; font-weight: 800;'>KSS Roadways ERP</h2>", unsafe_allow_html=True)
...
# --- Horizontal Nav ---
st.markdown("<h3 style='margin-bottom: 20px; font-weight: 800; color: #111827;'>KSS Roadways <span style='font-size: 12px; font-weight: 600; padding: 4px 8px; background: #E5E7EB; border-radius: 4px; vertical-align: middle;'>Dashboard</span></h3>", unsafe_allow_html=True)

nav1, nav2, nav3 = st.columns([7.5, 1.5, 1.0])
with nav1:
    MODULE_LIST = ["Dashboard / Fleet Status", "Trip Dispatch Entry", "POD Receive & Close", "Diesel Logs", "Driver Advances", "Modify Trips & Claims", "Driver Settlement", "Master Configuration", "Executive Retention Analytics", "Audit Log"] if st.session_state.user_role == "MASTER" else ["Dashboard / Fleet Status", "Driver Settlement", "Executive Retention Analytics"]
    menu = st.selectbox("Navigation", MODULE_LIST, label_visibility="collapsed")
with nav3:
...
if menu == "Dashboard / Fleet Status":
    ...
    if vehicles:
        ...
        # Calculate Cashflow (Pending Advances/Balances could go here, for now dummy 0 if not calculated)
        
        # Map statuses
        stat_counts = df_v['current_status'].value_counts().to_dict()
        def get_count(key): return stat_counts.get(key, 0)
        ...
        <div class="wm-card" style="background: #FEF2F2; border-color: #FEE2E2;">
            ...
            <div class="wm-metric-val" style="color: #7F1D1D; font-size: 2rem;">₹ {month_rev:,.2f}</div>
        </div>

        <div class="wm-card">
            <div class="wm-card-title">Vehicle Status Monitor</div>
            <div class="wm-status-grid">
                <div class="wm-status-pill"><div class="wm-status-badge">{get_count('IN_TRANSIT')}</div> In Transit</div>
                <div class="wm-status-pill"><div class="wm-status-badge">{get_count('WAITING_FOR_LOAD')}</div> Plant Loading</div>
                <div class="wm-status-pill"><div class="wm-status-badge">{get_count('WAITING_FOR_UNLOAD')}</div> Site Unloading</div>
                <div class="wm-status-pill"><div class="wm-status-badge">{get_count('AVAILABLE_FOR_LOAD')}</div> Ready / Available</div>
                <div class="wm-status-pill"><div class="wm-status-badge">{get_count('DRIVER_UNAVAILABLE')}</div> No Driver / Leave</div>
            </div>
            ...
            <div class="wm-workshop-box">
                ...
                <div class="wm-workshop-row">
                    ...
                    <span class="text-orange">{get_count('WORKSHOP_MAINTENANCE')}</span>
                </div>
                <div class="wm-workshop-row">
                    <span>Available Assets</span>
                    <span class="text-blue">{total_veh - get_count('WORKSHOP_MAINTENANCE') - get_count('DRIVER_UNAVAILABLE')}</span>
                </div>
            </div>
        </div>
        """
    ...
    with st.form("dispatch_form"):
        st.markdown('<div class="wm-card-title">Initiate Trip Dispatch</div>', unsafe_allow_html=True)
    ...
    with col_side:
        st.markdown('<div class="wm-card" style="background:#FEF2F2; border-color:#FEE2E2;"><div class="wm-card-title" style="color:#991B1B;">Pending POD List</div>', unsafe_allow_html=True)
        ...
        if not active_trips: st.info("No trips pending closure.")
            ...
            if sel_lr != "-- SELECT LR --":
                ...
                with st.form("pod_form"):
                    st.markdown('<div class="wm-card-title">Record POD & Settle Trip</div>', unsafe_allow_html=True)
    ...
    with hd1: st.markdown('<div class="section-header">Diesel Fuel Management</div>', unsafe_allow_html=True)
    with hd2:
                    ...
                    if st.form_submit_button("💾 Commit Updates", type="primary"):
                        if e_litres <= 0: show_error_toast("Fuel > 0 required.")
                            ...
                            def execute_fuel_edit():
                                ...
                                trigger_toast_and_rerun("SUCCESS", f"Updated.")
    ...
    with tab_audit_fuel:
        ...
        with df_col2: sel_fl_trk = st.selectbox("Truck No", ["All Trucks"] + sorted([v['vehicle_number'] for v in vehicles]))
    ...
    with col_a2:
        ...
        if adv_recs:
            ...
            with del_a1: del_adv_id = st.selectbox("Remove Entry", df_adv_recs['advance_id'].tolist())
            with del_a3:
                st.write(""); 
    ...
    with hm1: st.markdown('<div class="section-header">Modify Active & Closed Trips</div>', unsafe_allow_html=True)
    with hm2:
        current_d_rate = get_cached_diesel_rate()
        d_rate_fast = st.number_input("⛽ Active Diesel Rate (₹/L)*", value=current_d_rate, step=0.1)
        if d_rate_fast != current_d_rate: set_saved_diesel_rate(d_rate_fast)
    ...
    with f_c2: status_filter = st.selectbox("Filter Status", ["All Statuses", "IN_TRANSIT", "COMPLETED"])
    ...
    if not all_matched_trips: st.info("No trips found.")
        ...
        sel_t_key_label = st.selectbox("Target Trip Record", ["-- SELECT TRIP --"] + list(trip_map.keys()), index=0)
        ...
        if sel_t_key_label != "-- SELECT TRIP --":
            ...
            with st.form("mod_full_form"):
                ...
                if sel_route_choice != "-- MANUAL / SPOT ROUTE --":
                    ...
                    with c_spot2: auto_rate_mt = st.number_input("Spot Rate/MT*", min_value=0.0, step=25.0, value=0.0); auto_km = st.number_input("Standard KM", min_value=0.0, step=10.0, value=float(t_data['total_km_run'] or 0.0))

                st.markdown("<hr style='margin:10px 0;'>", unsafe_allow_html=True)
                ok1, ok2, ok3 = st.columns(3)
                with ok1: e_start_km = st.number_input("Start KM*", min_value=0.0, value=float(t_data['start_km'] or 0.0), step=10.0)
                with ok2: e_end_km = st.number_input("End KM", min_value=0.0, value=float(t_data['end_km'] or 0.0), step=10.0)
                with ok3:
                ...
                with f2: e_freight = st.number_input("Freight (₹)", value=round(e_ton * auto_rate_mt, 2), step=100.0)
                with f3: e_bata = st.number_input("Bata (₹)", value=float(t_data['driver_bata'] or 0.0), step=100.0)
                with f4: e_adv = st.number_input("Advance (₹)", value=float(t_data['cash_advance_issued'] or 0.0), step=500.0)
                with f5:
                    e_fuel_l = st.number_input("Diesel (L)", value=float(t_data['fuel_litres'] or 0.0), step=5.0)
                ...
                if st.form_submit_button("💾 Commit Updates", type="primary"):
                    def execute_mod_trip():
                        try:
                            f_c = round(e_fuel_l * d_rate_fast, 2)
            ...
            with act1:
                if t_data['trip_status'] == 'COMPLETED' and st.button("🔓 Reopen Trip (Set back to IN_TRANSIT)", use_container_width=True):
    ...
    if drivers:
        ...
        st.markdown(f'<div class="section-header">Settlement Statement</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="wm-card" style="background:#EEF2FF; border-color:#E0E7FF;"><div class="wm-card-title" style="color:#3730A3;">Overall Cycle Position</div>', unsafe_allow_html=True)
        ...
        with act_col2:
            ...
            else: st.error("⚠️ Install fpdf via terminal")
                
        with act_col3:
            if st.session_state.user_role == "MASTER":
                if st.button("Mark Settled", type="primary", use_container_width=True):
                    confirm_action_dialog(f"mark records for {d_dict[sel_d_name]['full_name']} as SETTLED", lambda: (run_query("UPDATE trips SET settlement_status='SETTLED' WHERE primary_driver_id=%s AND trip_start_date::date>=%s AND trip_start_date::date<=%s", (d_id, s_from, s_to), fetch=False), run_query("UPDATE driver_direct_advances SET is_settled=TRUE WHERE driver_id=%s AND advance_date::date>=%s AND advance_date::date<=%s", (d_id, s_from, s_to), fetch=False), trigger_toast_and_rerun("SUCCESS", "Settled.")))

elif menu == "Master Configuration":
    t_v, t_d, t_r, t_b = st.tabs(["Trucks", "Drivers", "Freight Slabs", "Bata Slabs"])
    ...
    with t_r:
        ...
        with c1:
            with st.form("quick_slab"):
                ...
                km = st.number_input("Std KM", min_value=0.0, step=10.0)
                if st.form_submit_button("Save Route", type="primary"):
                    if not dt or rt <= 0: show_error_toast("Invalid inputs.")
                    else: confirm_action_dialog(f"save slab", lambda: ([run_query("INSERT INTO destinations_freight_master (cargo_type, origin, destination_name, capacity_tons, freight_rate_per_ton, standard_km) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (cargo_type, origin, destination_name, capacity_tons) DO UPDATE SET freight_rate_per_ton = EXCLUDED.freight_rate_per_ton, standard_km = EXCLUDED.standard_km;", (cg, so, dt, c, rt, km), fetch=False) for c in ([25.0, 30.0] if cg == "BAG" else [cl])], get_cached_routes.clear(), trigger_toast_and_rerun("SUCCESS", "Saved.")))
        with c2:
    ...
    with t_b:
        ...
        with c1:
            with st.form("quick_bata"):
                ...
                ba = st.number_input("Bata (₹)*", min_value=0.0, step=100.0, value=3000.0)
    ...
    with af_col5: search_aud = st.text_input("Search LR").strip().upper()
